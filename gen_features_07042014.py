...
loc_out_train = "//home//harini//Kaggle_data//shop//train_07042014.csv"
loc_out_test = "//home//harini//Kaggle_data//shop//test_07042014.csv"
...
def generate_new_dict():
	...
	features['has_bought_b'] = 0 
	
	features['has_bought_category_q'] = 0 
	features['has_bought_c_q'] = 0 
	features['has_bought_b_q'] = 0 

	features['has_bought_category_a'] = 0 
	features['has_bought_c_a'] = 0 
	features['has_bought_b_a'] = 0 		

	#Repeat all the above for 30, 60, 90, 180 days
	#Booleans
	features['has_bought_category_30'] = 0 
	features['has_bought_c_30'] = 0 
	features['has_bought_b_30'] = 0 
	
	features['has_bought_category_q_30'] = 0 
	features['has_bought_c_q_30'] = 0 
	features['has_bought_b_q_30'] = 0 
	
	features['has_bought_category_a_30'] = 0 
	features['has_bought_c_a_30'] = 0 
	features['has_bought_b_a_30'] = 0 	
	
	# 60
	#Booleans
	features['has_bought_category_60'] = 0 
	features['has_bought_c_60'] = 0 
	features['has_bought_b_60'] = 0 
	
	features['has_bought_category_q_60'] = 0 
	features['has_bought_c_q_60'] = 0 
	features['has_bought_b_q_60'] = 0 
	
	features['has_bought_category_a_60'] = 0 
	features['has_bought_c_a_60'] = 0 
	features['has_bought_b_a_60'] = 0 

	# 90
	#Booleans
	features['has_bought_category_90'] = 0 
	features['has_bought_c_90'] = 0 
	features['has_bought_b_90'] = 0 
	
	features['has_bought_category_q_90'] = 0 
	features['has_bought_c_q_90'] = 0 
	features['has_bought_b_q_90'] = 0 
	
	features['has_bought_category_a_90'] = 0 
	features['has_bought_c_a_90'] = 0 
	features['has_bought_b_a_90'] = 0 

	# 180
	#Booleans
	features['has_bought_category_180'] = 0 
	features['has_bought_c_180'] = 0 
	features['has_bought_b_180'] = 0 
	
	features['has_bought_category_q_180'] = 0 
	features['has_bought_c_q_180'] = 0 
	features['has_bought_b_q_180'] = 0 
	
	features['has_bought_category_a_180'] = 0 
	features['has_bought_c_a_180'] = 0 
	features['has_bought_b_a_180'] = 0 
		...
		for e, line in enumerate( open(loc_transactions) ):
			if e > 0: #skip header
				...
				if last_id != row[0] and e != 1:
					...
					if features['has_bought_c'] > 0 and features['has_bought_b'] > 0:
						features['has_bought_cb'] = 1
				...
				if row[0] in train_ids or row[0] in test_ids:
					...
					if offer_company == trans_company:
						features['has_bought_c'] = 1.0
						features['has_bought_c_q'] += float( row[9] )
						features['has_bought_c_a'] += float( row[10] )
						if date_diff_days < 30:
							features['has_bought_c_30'] = 1.0
							features['has_bought_c_q_30'] += float( row[9] )
							features['has_bought_c_a_30'] += float( row[10] )
						if date_diff_days < 60:
							features['has_bought_c_60'] = 1.0
							features['has_bought_c_q_60'] += float( row[9] )
							features['has_bought_c_a_60'] += float( row[10] )
						if date_diff_days < 90:
							features['has_bought_c_90'] = 1.0
							features['has_bought_c_q_90'] += float( row[9] )
							features['has_bought_c_a_90'] += float( row[10] )
						if date_diff_days < 180:
							features['has_bought_c_180'] = 1.0
							features['has_bought_c_q_180'] += float( row[9] )
							features['has_bought_c_a_180'] += float( row[10] )

					# if customer has shopped for brand before
					if offer_brand == trans_brand:
						features['has_bought_b'] = 1.0
						features['has_bought_b_q'] += float( row[9] )
						features['has_bought_b_a'] += float( row[10] )
						if date_diff_days < 30:
							features['has_bought_b_30'] = 1.0
							features['has_bought_b_q_30'] += float( row[9] )
							features['has_bought_b_a_30'] += float( row[10] )
						if date_diff_days < 60:
							features['has_bought_b_60'] = 1.0
							features['has_bought_b_q_60'] += float( row[9] )
							features['has_bought_b_a_60'] += float( row[10] )
						if date_diff_days < 90:
							features['has_bought_b_90'] = 1.0
							features['has_bought_b_q_90'] += float( row[9] )
							features['has_bought_b_a_90'] += float( row[10] )
						if date_diff_days < 180:
							features['has_bought_b_180'] = 1.0
							features['has_bought_b_q_180'] += float( row[9] )
							features['has_bought_b_a_180'] += float( row[10] )
					...
					features['offer_time_from_peak'] = min(abs(offer_month - float(seasonality[ offer_category])),abs(13 - offer_month +float(seasonality[ offer_category])))
