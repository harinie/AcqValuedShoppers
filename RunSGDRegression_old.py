...
def svm_cross_validate_category(X,y,category,C,penalty,sample_weights):
	...
	clf_svm_1 = SGDRegressor(loss=loss, penalty=penalty, epsilon=epsilon,alpha=C, shuffle=True)
	clf_svm_2 = SGDRegressor(loss=loss, penalty=penalty, epsilon=epsilon,alpha=C, shuffle=True)
	...
	for i in range(0,len(u)):
		...
		if(np.sum(test_ids & i_inds)!=0):
			fpr, tpr, thresholds = metrics.roc_curve(y[test_ids & i_inds], y_1[i_inds[test_ids]], pos_label=1)
			auc[0,i] = metrics.auc(fpr, tpr)

		if(np.sum(train_ids & i_inds)!=0):
			fpr, tpr, thresholds = metrics.roc_curve(y[train_ids & i_inds], y_2[i_inds[train_ids]], pos_label=1)
...
#for i, C in enumerate(10. ** np.arange(-4,4)):
   ...
   #cv_auc =svm_cross_validate(X,y,category,C,'l1',sample_weights)
...
for i, C in enumerate(10. ** np.arange(-4,4)):
    # turn down tolerance for short training time
    clf_svm = SGDRegressor(loss=loss, penalty='l2', epsilon=epsilon, alpha=C, shuffle=True)
...
clf_svm = SGDRegressor(loss=loss, penalty='l2', epsilon=epsilon, alpha=100, shuffle=True)
clf_svm.fit(X,y,sample_weight=sample_weights)
...
np.savetxt("testsubmission_full_huber_05172014.csv", submission, fmt="%d,%1.6f")
