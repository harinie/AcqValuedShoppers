...
loc_train = "//home//harini//Kaggle_data//shop//train_05162014.csv"
loc_test = "//home//harini//Kaggle_data//shop//test_05162014.csv"
...
def svm_cross_validate_category(X,y,category,C,penalty,sample_weights):
	...
	clf_svm_1 = svm.SVC(C=C, probability=True)
	clf_svm_2 = svm.SVC(C=C, probability=True)
...
clf_svm = svm.SVC(C=C, probability=True)
clf_svm.fit(X,y,sample_weight=sample_weights)
coef_svm = clf_svm.coef_.ravel()
	
count=0
outline=""
out_coeffs = open("coeffs_hinge_05162014_l2.csv", "w+")
for e,x in enumerate(header):
	if (x == "label" or x == "id" or x == "category" or x ==""):
		print(e,x,"nan")
		count += 1
...
np.savetxt("testsubmission_hinge_05162014.csv", submission, fmt="%d,%1.6f")
