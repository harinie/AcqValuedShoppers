...
clf_LR = LogisticRegression(C=1, penalty='l2', tol=0.01)
clf_LR.fit(X,y)
coef_LR = clf_LR.coef_.ravel()
	
count=0
outline=""
out_coeffs = open("coeffs_05162014_logistic_l2.csv", "w+")
for e,x in enumerate(header):
	if (x == "label" or x == "id" or x == "category" or x ==""):
		print(e,x,"nan")
		count += 1
...
np.savetxt("testsubmission_logistic_05162014.csv", submission, fmt="%d,%1.6f")
